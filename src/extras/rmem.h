...
typedef struct ObjPool {
    uintptr_t mem, offs;
    int objSize, freeBlocks, memSize;
} ObjPool;
...
//------------------------------------------------------------------------------------
RMEMAPI MemPool CreateMemPool(int bytes);
RMEMAPI MemPool CreateMemPoolFromBuffer(void *buf, int bytes);
RMEMAPI void DestroyMemPool(MemPool *mempool);

RMEMAPI void *MemPoolAlloc(MemPool *mempool, int bytes);
RMEMAPI void *MemPoolRealloc(MemPool *mempool, void *ptr, int bytes);
...
RMEMAPI int GetMemPoolFreeMemory(const MemPool mempool);

//------------------------------------------------------------------------------------
// Functions Declaration - Object Pool
//------------------------------------------------------------------------------------
RMEMAPI ObjPool CreateObjPool(int objsize, int len);
RMEMAPI ObjPool CreateObjPoolFromBuffer(void *buf, int objsize, int len);
...
//------------------------------------------------------------------------------------
RMEMAPI BiStack CreateBiStack(int len);
RMEMAPI BiStack CreateBiStackFromBuffer(void *buf, int len);
RMEMAPI void DestroyBiStack(BiStack *destack);

RMEMAPI void *BiStackAllocFront(BiStack *destack, int len);
RMEMAPI void *BiStackAllocBack(BiStack *destack, int len);
...
//----------------------------------------------------------------------------------
static inline int __AlignSize(const int size, const int align)
{
    return (size + (align - 1)) & -align;
}

static MemNode *__SplitMemNode(MemNode *const node, const int bytes)
{
...
static MemNode *__FindMemNode(AllocList *const list, const int bytes)
{
...
MemPool CreateMemPool(const int size)
{
    MemPool mempool = { 0 };
...
MemPool CreateMemPoolFromBuffer(void *const restrict buf, const int size)
{
    MemPool mempool = { 0 };
...
void *MemPoolAlloc(MemPool *const mempool, const int size)
{
    if ((size == 0) || (size > mempool->arena.size)) return NULL;
    else
    {
        MemNode *new_mem = NULL;
        const int ALLOC_SIZE = __AlignSize(size + sizeof *new_mem, sizeof(intptr_t));
        const int BUCKET_SLOT = (ALLOC_SIZE >> MEMPOOL_BUCKET_BITS) - 1;
...
void *MemPoolRealloc(MemPool *const restrict mempool, void *const ptr, const int size)
{
...
void MemPoolFree(MemPool *const restrict mempool, void *const ptr)
{
    ...
    else
    {
        ...
        const int BUCKET_SLOT = (mem_node->size >> MEMPOOL_BUCKET_BITS) - 1;
...
int GetMemPoolFreeMemory(const MemPool mempool)
{
    int total_remaining = mempool.arena.offs - mempool.arena.mem;

    for (MemNode *n=mempool.large.head; n != NULL; n = n->next) total_remaining += n->size;

    for (int i=0; i<MEMPOOL_BUCKET_SIZE; i++) for (MemNode *n = mempool.buckets[i].head; n != NULL; n = n->next) total_remaining += n->size;
...
ObjPool CreateObjPool(const int objsize, const int len)
{
    ObjPool objpool = { 0 };
    if ((len == 0) || (objsize == 0)) return objpool;
    else
    {
        const int aligned_size = __AlignSize(objsize, sizeof(int));
        ...
        {
            int *const restrict index = ( int* )(objpool.mem + (i*aligned_size));
            *index = i + 1;
        }
...
ObjPool CreateObjPoolFromBuffer(void *const restrict buf, const int objsize, const int len)
{
    ObjPool objpool = { 0 };

    // If the object size isn't large enough to align to a int, then we can't use it.
    const int aligned_size = __AlignSize(objsize, sizeof(int));
    if ((buf == NULL) || (len == 0) || (objsize < sizeof(int)) || (objsize*len != aligned_size*len)) return objpool;
    else
    {
        objpool.objSize = aligned_size;
        objpool.memSize = objpool.freeBlocks = len;
...
void *ObjPoolAlloc(ObjPool *const objpool)
{
    if (objpool->freeBlocks > 0)
    {
        ...
        // Head = &pool[0];
        // ret = Head == ret = &pool[0];
        int *const restrict block = ( int* )objpool->offs;
        objpool->freeBlocks--;
...
void ObjPoolFree(ObjPool *const restrict objpool, void *const ptr)
{
    ...
    else
    {
        ...
        // Head = p;
        int *const restrict index = ( int* )block;
...
//----------------------------------------------------------------------------------
BiStack CreateBiStack(const int len)
{
    BiStack destack = { 0 };
...
BiStack CreateBiStackFromBuffer(void *const buf, const int len)
{
    BiStack destack = { 0 };
...
void *BiStackAllocFront(BiStack *const restrict destack, const int len)
{
    if (destack->mem == 0) return NULL;
    else
    {
        const int ALIGNED_LEN = __AlignSize(len, sizeof(uintptr_t));
...
void *BiStackAllocBack(BiStack *const restrict destack, const int len)
{
    if (destack->mem == 0) return NULL;
    else
    {
